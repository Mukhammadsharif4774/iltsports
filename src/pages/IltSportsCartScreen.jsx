...
import IltSportsCartItemComponent from '../components/IltSportsCartItemComponent';
...
export default function () {
  ...
  const handleOrder = () => {
    const destinationScreen = cart.length
      ? 'MarsSportsCartSuccessScreen'
      : 'MarsSportsHomeScreen';
        ...
        <>
          <View style={{height: height * 0.7}}>
            <ScrollView style={styles.flex} contentContainerStyle={styles.main}>
              {cart.map((item, index) => (
                <IltSportsCartItemComponent item={item} key={index} />
              ))}
            </ScrollView>
          </View>
        </>
